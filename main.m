...
for k = 1:N-1
    ...
    q_e_dot(:,k) = 0.5 * M * (alpha_2 + omega_v(:,k));
    M_dot = quatM(q_e_dot(:,k));
    omega_v_dot = -2*k3*(M_dot.' * q_e(:,k) + M.' * q_e_dot(:,k));
    % tau
    tau = cross(omega_k,I*omega_k)+I*omega_v_dot-k4*alpha_2-0.5*M.'*q_e;
    alpha_2_dot(:,k) = I\(-cross(omega_k,I*omega_k)+tau)-omega_v_dot;
